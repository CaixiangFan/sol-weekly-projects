...
contract Token is ERC721A, Ownable, ReentrancyGuard {
    ...
    function mintTokens(uint256 _numberOfTokensRequested)
        public
        payable
        nonReentrant
    {
        ...
        require(
            totalSupply() + _numberOfTokensRequested <= MAX_TOTAL_SUPPLY,
        ...
        if (alreadyMintedFreeByWallet[msg.sender]) {
            ...
            require(
                numberOfPaidMintByWallet[msg.sender] +
                    _numberOfTokensRequested <=
                    WALLET_MAX_PAID_MINT,
                "Exceeded paid mint limit"
            );
            // Are ethers paid enough?
            require(
                msg.value >= _numberOfTokensRequested * MINT_PRICE,
            ...
            if (currentTotalFreeMint < MAX_FREE_SUPPLY) {
                ...
                require(
                    msg.value >= (_numberOfTokensRequested - 1) * MINT_PRICE,
                    "Not enough ether paid"
                );
                currentTotalFreeMint++;
                alreadyMintedFreeByWallet[msg.sender] = true;
                numberOfPaidMintByWallet[
                    msg.sender
                ] += (_numberOfTokensRequested - 1);
                _safeMint(msg.sender, _numberOfTokensRequested);
            } else {
    ...
    /// @param myTokenId id of token to be burned
    function burnMyToken(uint256 myTokenId) public {
